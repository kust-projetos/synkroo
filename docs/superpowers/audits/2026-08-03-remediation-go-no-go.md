...
| integration/PostgreSQL | PASS | Final bounded run after deterministic outbox fixture repair: 32 suites and 195 tests passed; migration 0019 applied before execution. Targeted Evolution tenancy cases pass for unknown, disabled, enabled-owner and other-clinic installations. |
...
## Auditor follow-up — 2026-08-10
The final independent audit blockers are addressed in the current tree: production `npm audit --omit=dev --audit-level=high` exits 0 after postcss 8.5.26 pulled nanoid 3.3.18; charge and campaign dispatch propagate stable outbox business keys as provider `Idempotency-Key`; `worker-entry.mjs` and `wrangler.toml` provide a five-minute Cloudflare Cron consumer for `/api/cron/outbox`; Evolution deduplication uses PostgreSQL provider/event uniqueness with atomic `ON CONFLICT DO NOTHING`; disabled channel installations are rejected; and unknown Evolution instances no longer fall back to the first active clinic. The complete PostgreSQL integration run now passes 32/32 suites and 195/195 tests after the deterministic outbox fixture provisions its clinic parent. ADR-BASE-13, the master plan, ledger and E2E count note are reconciled; the final review explicitly marks its historical NO-GO as superseded by the 2026-08-10 GO decision.
