...
describe('resolveAccess', () => {
  ...
  it('owner role bypasses within instance', async () => {
...
export function buildPresetPermissions(preset: PresetDef): string[] {
  ...
  for (const p of catalog) if (preset.modules.includes(p.module)) keys.add(p.key);
  for (const k of preset.extraKeys ?? []) keys.add(k);
  return [...keys];
}
