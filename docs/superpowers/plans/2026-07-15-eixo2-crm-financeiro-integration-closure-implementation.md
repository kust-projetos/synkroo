...
- [ ] **GREEN:** Set `financeiroManifest.jobs = ['financeiro-collections']`. Move existing page JSX to `financeiro-client.tsx`; server page checks `moduleManifest.isEnabled('financeiro')`, calls `notFound()` when false, else renders client. Add typed hooks: `useFinanceDashboard` -> `/api/financeiro/dashboard`; `useFinanceBudgets` -> `/api/financeiro/budgets`; `useFinanceCharges` -> `/api/financeiro/charges`; `useFinanceGateways` -> `/api/financeiro/gateways`; `useBudgetPayments(id)` -> `/api/financeiro/budgets/:id/payments`. Pass results to the matching tab and implement loading/error/empty states. Do not gate provider webhook with `withModuleRoute`.
