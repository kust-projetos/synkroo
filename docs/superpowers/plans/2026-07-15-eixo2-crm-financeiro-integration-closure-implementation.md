...
- [ ] **RED:** In `page.test.tsx`, mock manifest disabled and expect `notFound()`. Add `src/components/financeiro/__tests__/financeiro-client.test.tsx`: mock actual responses: dashboard `{ overdueCount,totalOverdue,overdueStages }`, budgets `{ data,total }`, collections `{ data,total,page,limit }`, gateways `{ data }`, budget payments `{ data }`; expect displayed data, not null props or placeholders. In webhook test, seed/mock a known charge and expect reconciliation; unknown provider/charge returns 404. Run: `npx jest src/app/dashboard/financeiro/page.test.tsx src/components/financeiro/__tests__/financeiro-client.test.tsx src/app/api/financeiro/webhooks/[provider]/route.test.ts --runInBand`. Expected: FAIL because page is client-only, dashboard props are empty, and tabs contain placeholders.
- [ ] **GREEN:** Set `financeiroManifest.jobs = ['financeiro-collections']`. Move existing page JSX to `financeiro-client.tsx`; server page checks `moduleManifest.isEnabled('financeiro')`, calls `notFound()` when false, else renders client. Replace stale `DashboardMetrics` UI contract with `FinanceDashboardSummary={overdueCount:number;totalOverdue:number;overdueStages:{light:number;firm:number;internal:number}}`. Add hooks: `useFinanceDashboard` -> `/api/financeiro/dashboard`; `useFinanceBudgets` -> `/api/financeiro/budgets` mapping `data`; `useFinanceCollections` -> `/api/financeiro/collections` mapping `data`; `useFinanceGateways` -> `/api/financeiro/gateways` mapping `data`; `useBudgetPayments(id)` -> `/api/financeiro/budgets/:id/payments` mapping `data`. Pass summary/budgets/collections/gateways to matching tabs and load payments only after a selected budget. Implement loading/error/empty states. Do not invent `GET /charges` and do not gate provider webhook with `withModuleRoute`.
- [ ] **VERIFY:** `npx jest src/app/dashboard/financeiro/page.test.tsx src/components/financeiro/__tests__/financeiro-client.test.tsx src/app/api/financeiro/webhooks/[provider]/route.test.ts --runInBand` → PASS.
...
- [ ] **GREEN:** Add dynamic imports and idempotent registry/catalog registration for both CRM and Financeiro in `bootstrap.ts`; include both manifests in menu. Create `preset-policy.json` as canonical source with `modulePermissions.crm` and `.financeiro` arrays of `{ key,module,label }`, plus role module/extra-key policies. Make `presets.ts` import JSON and build `SYSTEM_PRESETS`. In `backfill(client)`, read JSON with `readFile(new URL('../src/core/rbac/preset-policy.json', import.meta.url), 'utf8')` and `JSON.parse`; first insert permission rows `(key,module,label)` from policy, then expand staff module keys from DB catalog, and grant Owner every non-master catalog key. Export `backfill(client)`, retain CLI `main()`, and use parameterized `ON CONFLICT DO NOTHING` inserts.
...
- WhatsApp replies remain app-side through `webhook-router.ts -> enviarMensagem`; no Atendimento action is in the IA tool allowlist by design.
