# Phase 3: Sidebar — Heroicons, Sections, Collapse, Theme-Aware

**Goal:** Reescrever completamente a sidebar com Heroicons, 3 seções (Principal/Comunicação/Gestão), toggle collapse/expand, badges contextuais, e suporte a tema claro/escuro.
...
**Files:**
- Modify: `src/lib/ui/sidebar.tsx` (complete rewrite)

- [ ] **Step 1: Rewrite `src/lib/ui/sidebar.tsx`**

Replace the entire file (currently ~250 lines). The new sidebar must:

1. Use Heroicons instead of inline SVGs
2. Have 3 sections: Principal, Comunicação, Gestão
3. Support collapse/expand (240px / 64px)
4. Persist state in localStorage (`synkroo_sidebar_collapsed`)
5. Theme-aware: white in light, dark zinc in dark
6. Show badges for notifications (patients count, today appointments, unread messages, etc.)
7. Mobile: sheet overlay (preserves existing mobile behavior)
8. User profile at bottom with avatar + name + role + logout
          ...
          {!collapsed && (
            <>
              <div className="flex-1 min-w-0">
                <div className="text-xs font-semibold text-foreground truncate">{profile?.full_name || "Usuario"}</div>
                <div className="text-[10px] text-zinc-400">Admin</div>
              </div>
              <button onClick={() => signOut()} className="text-zinc-400 hover:text-zinc-600 dark:hover:text-zinc-300 transition-colors">
...
Expected: Build succeeds. The sidebar file compiles without errors.

- [ ] **Step 3: Commit**

```bash
git add src/lib/ui/sidebar.tsx
git commit -m "feat: rewrite sidebar with Heroicons, 3 sections, collapse, theme-aware"
```
...
- Collapsible (240px/64px) with localStorage persistence
- Organized in 3 sections (Principal, Comunicao, Gestao)
- Using Heroicons instead of inline SVGs
- With contextual badges
- With theme toggle in footer
