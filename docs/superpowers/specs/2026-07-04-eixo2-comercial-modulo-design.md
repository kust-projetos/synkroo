...
| Notificação de lead quente | **Escopo via nova Action E-01 + job E-05** | E-05 decide/dedup; Atendimento envia mensagem direta para staff |
...
| REQ-COM-03 | When a lead score crosses the hot threshold, the Comercial module shall enqueue a hot-lead notification, deduplicate it for 24h, resolve a staff recipient, and send through `atendimento.enviarMensagemDireta`. | E-05-S04 |
...
4. Comercial agenda avaliação via service interno `ensurePacienteParaLead`, chamando Actions E-02 `operacional.criarPaciente`/`operacional.atualizarPaciente` e `operacional.agendarConsulta`, depois `converterLead`.
5. Comercial notifica lead quente via Action `atendimento.enviarMensagemDireta` (nova action E-01 para `channel + externalId + message`); não usa `@/lib/whatsapp/send` direto após migração.
...
|---|---|---|
| `leads` | Comercial | mover para `src/modules/comercial/schema/leads.ts`; adicionar `phoneNormalized`, backfill/preflight e unique `(clinicId, phoneNormalized)` |
...
| `agendarAvaliacao` | `comercial:convert_leads` | orquestração | service interno garante paciente via `operacional.criarPaciente`/`atualizarPaciente`, agenda via `operacional.agendarConsulta`, converte após sucesso |
| `listarPipeline` | `comercial:view` | leitura | Kanban |
| CRUD pipeline stages | `comercial:manage_pipeline` | escrita | stages da clínica |
| CRUD tasks comerciais | `comercial:manage_leads` | escrita/leitura | action-only neste slice; follow-up manual por lead, sem nova UI |
Manifesto:
```ts
...
| Fase | Conteúdo | Gate |
|---|---|---|
| F1 Schema seam | Mover `leads`, `leadActivities`, `pipelineStages`, `tasks`; backfill/preflight `phoneNormalized`; adicionar dedup key | `typecheck` + `db:generate` sem drift inesperado |
...
| F5 Cross-module bridges | Atendimento→Comercial; Comercial→Operacional; `src/app/api/cron/hot-leads/route.ts` → `runComercialSystemAction(processarNotificacoesLeadsQuentes)` → `atendimento.enviarMensagemDireta` | integration focused |
...
2. Backfill de `phoneNormalized` antes da constraint; preflight detecta duplicatas existentes por clínica.
3. Merge/reopen duplicatas antes da unique: escolher vencedor por `convertedAt DESC NULLS LAST, updatedAt DESC, createdAt DESC`; migrar activities/tasks para o vencedor; marcar perdedores como `lost` e setar `phoneNormalized = null`.
4. Enforçar unique DB `(clinicId, phoneNormalized)` quando `phoneNormalized` não é nulo.
5. Usar upsert/transação para capturas concorrentes; corrida webhook+manual atualiza a mesma linha.
6. Se houver lead convertido/perdido, reabrir/atualizar o lead existente; não criar 2º lead automático.
7. Nunca deduplicar entre clínicas.
---
...
| Comercial | Operacional | `agendarAvaliacao` usa service interno para chamar `operacional.criarPaciente`/`operacional.atualizarPaciente`, depois `operacional.agendarConsulta`, e só então converte lead |
| Comercial | Atendimento | job `comercial.hot-leads` chama nova action `atendimento.enviarMensagemDireta` (`channel`, `externalId`, `message`); sem `sendWhatsAppMessage` direto |
...
- Notification mode: assíncrono via `src/app/api/cron/hot-leads/route.ts`, gated por `assertModuleForJob('comercial')`, executando action `comercial.processarNotificacoesLeadsQuentes` via `runComercialSystemAction`; capture/qualify só enfileira/loga.
---
## 11. Testes
**Tests:**
...
- Contract (auto if external HTTP): `atendimento.enviarMensagemDireta` and Operacional scheduling Action via mocked/integration `runAction`.
...
- [ ] Leads/pipeline/tasks ownership moved with `phoneNormalized` backfill/preflight + DB dedup key.
...
- [ ] Hot lead notification dedup uses `leadActivities` 24h marker and sends through `atendimento.enviarMensagemDireta`, not permanent no-op.
- [ ] `agendarAvaliacao` uses internal service to call `operacional.criarPaciente`/`operacional.atualizarPaciente`, then `operacional.agendarConsulta`, and updates lead only after success.
- [ ] Comercial task actions cover create/list/update/close within lead scope; no new seller UI in this slice.
- [ ] `src/app/api/cron/hot-leads/route.ts` runs `comercial.processarNotificacoesLeadsQuentes` via `runComercialSystemAction`, with `assertModuleForJob('comercial')` and CRON_SECRET.
...
| Notification usa WhatsApp direto | Criar/usar Action E-01 `atendimento.enviarMensagemDireta` |
| Dedup sem constraint única | Backfill/preflight, merge duplicatas para vencedor, setar perdedores `phone_normalized = null`, adicionar unique + upsert |
