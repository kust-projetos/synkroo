...
- **Lifecycle do handle (achado round 3 #1 + correção contrato):** o handle é um **`sessionToken` reutilizável** durante sua validade — um turno do agente faz **várias** execuções (loop multi-tool) com o **mesmo** handle. Vínculo **estrito** handle ↔ `conversationId` ↔ `principalRef` ↔ `source` (o app rejeita uso cruzado). **Expiração curta** + renovação pelo app. **Revogação** (encerrar sessão invalida o handle).
- **Anti-replay por AÇÃO, não por handle:** cada `executeAction` carrega uma `idempotencyKey` (= o `tool_call_id` do LLM, único por ação). O `ia-bridge` deduplica por essa chave — retry da **mesma** ação dedupa; ações distintas no mesmo turno passam. O handle **não** é single-use (isso quebraria o loop). A idempotência de **mensagens** de canal já é coberta por `externalMessageId` no inbound (app-side).
