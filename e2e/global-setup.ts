...
export default async function globalSetup(_config: FullConfig) {
  ...
  try {
    ...
    if (!csrfResponse.ok()) throw new Error(`E2E setup CSRF failed: ${csrfResponse.status()}`)
    const { csrfToken } = await csrfResponse.json() as { csrfToken?: string }
    if (!csrfToken) throw new Error('E2E setup CSRF response did not include a token')
    ...
    if (!callbackResponse.ok()) {
      const body = (await callbackResponse.text()).slice(0, 500).replace(/\s+/g, ' ')
      throw new Error(`E2E setup credentials failed: status=${callbackResponse.status()}; body=${body}`)
    }
    ...
    if (page.url().includes('/login')) {
      ...
      throw new Error(`E2E setup login redirect failed: url=${page.url()}; body=${body}`)
    }
