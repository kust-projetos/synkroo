...
async function expectLeadData(page: Page) {
  const leadRows = page.locator(
    '[data-testid="lead-card"], table tbody tr, [class*="lead-card"]',
  );
...
test.describe("Leads Page", () => {
  ...
  test("should display lead rows when data exists", async ({ page }) => {
    const rows = page.locator(
      'table tbody tr, [data-testid="lead-card"], [class*="lead-card"]',
    );
  ...
  test("should have new lead button", async ({ page }) => {
    await expect(
      page.getByRole("button", { name: "Novo Lead", exact: true }),
    ).toBeVisible();
  });
  ...
  test("should open new lead form", async ({ page }) => {
    ...
    await expect(
      page.getByRole("heading", { name: "Novo Lead" }),
    ).toBeVisible();
    await expect(
      page.getByRole("button", { name: "Salvar Lead" }),
    ).toBeVisible();
  });
  test("should show lead score", async ({ page }) => {
    const score = page
      .locator('[data-testid*="score"], [class*="score"]')
      .first();
    await expect(score).toBeVisible({ timeout: 15000 });
  });

  test("should have filter controls for temperature and status", async ({
    page,
  }) => {
    await expect(page.getByText("Status", { exact: true })).toBeVisible();
    await expect(page.getByText("Temperatura", { exact: true })).toBeVisible();
  ...
  test("should open lead creation form with name and phone fields", async ({
    page,
  }) => {
...
test.describe("Leads status interaction", () => {
  test("should expose status controls for an existing lead", async ({
    page,
  }) => {
    await login(page);
    await page.goto(`${BASE_URL}/dashboard/leads`);
