...
test.describe("Patients Page", () => {
  ...
  test("should display patient list", async ({ page }) => {
    const patients = page.locator(
      'table tbody tr, [data-testid="patient-card"], [class*="patient-card"]',
    );
...
test.describe("Patient Detail", () => {
  ...
  test("should show patient detail page with tabs", async ({ page }) => {
    ...
    await expect(page.getByRole("tab", { name: /Informações/i })).toBeVisible();
    await expect(
      page.getByRole("tab", { name: /Agendamentos/i }),
    ).toBeVisible();
  });

  test("should display patient information or appointment empty state", async ({
    page,
  }) => {
    await openPatient(page);
    ...
    await expect(
      page.getByText(/Nenhum agendamento encontrado|Data\/Hora/).first(),
    ).toBeVisible();
  });

  test("should expose appointment creation from empty patient schedule", async ({
    page,
  }) => {
    await openPatient(page);
