...
async function expectSettingsContent(page: Page) {
  await expect(
    page.locator('form, [class*="card"], input, button').first(),
  ).toBeVisible();
}
...
test.describe("Settings Page", () => {
  ...
  test("should display settings page", async ({ page }) => {
    await expect(page.locator("h1, h2")).toContainText(
      /configurações|settings|preferências/i,
    );
  });

  test("should have settings navigation", async ({ page }) => {
    await expect(
      page.locator('nav, [role="navigation"], [role="tablist"], aside').first(),
    ).toBeVisible();
  });
});
...
test.describe("Reminder Settings", () => {
  ...
  test("should have notification settings", async ({ page }) => {
    await expect(
      page.locator('form, input, button, [class*="switch"]').first(),
    ).toBeVisible();
  });
});
