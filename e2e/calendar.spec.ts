...
// Authenticated project storageState supplies the canonical session. Keep the UI login fallback for local unauthenticated runs.
...
// Login through the canonical NextAuth UI flow when no shared state is available.
...
async function clickSlot(page: Page, hour: number, minute: number) {
  const slot = page
    .locator(`[data-hour="${hour}"][data-minute="${minute}"]`)
    .first();
...
test.describe("Calendar - Rendering", () => {
  ...
  test("should display calendar toolbar", async ({ page }) => {
    ...
    await expect(
      page.locator('button:has-text("Profissionais")').last(),
    ).toBeVisible();
  ...
  test("week view should render 7 day columns", async ({ page }) => {
    ...
    const dayHeaders = page
      .locator("main")
      .locator("text=/segunda|terça|quarta|quinta|sexta|sábado|domingo/i");
  ...
  test("week view should display event cards", async ({ page }) => {
    await switchView(page, "Semana");
    await expect(
      page.locator('[role="button"][aria-label*="- "]').first(),
    ).toBeVisible();
  });
...
test.describe("Calendar - Navigation", () => {
  ...
  test("view switcher should change active view", async ({ page }) => {
    ...
    const isActive = await semanaBtn.evaluate((el) =>
    ...
    const diaActive = await diaBtn.evaluate((el) =>
      el.classList.contains("bg-teal-600"),
    );
    expect(diaActive).toBeTruthy();

    const semanaInactive = await semanaBtn.evaluate(
      (el) => !el.classList.contains("bg-teal-600"),
    );
  ...
  test("view state should sync with URL", async ({ page }) => {
    ...
    const diaBtn = page.locator('button:has-text("Dia")');
    const isActive = await diaBtn.evaluate((el) =>
      el.classList.contains("bg-teal-600"),
    );
    expect(isActive).toBeTruthy();
  });

  test("month view click on day should switch to day view", async ({
    page,
  }) => {
    await switchView(page, "Mes");
    await page.waitForTimeout(500);
    ...
    const clicked = await page.evaluate(() => {
      ...
      const cells = document.querySelectorAll(
        'main div[class*="cursor-pointer"]',
      );
      for (const cell of cells) {
        if (
          cell.textContent?.trim() === "14" ||
          (cell as HTMLElement).innerText?.trim()?.match(/^14$/)
        ) {
      ...
      for (const el of allEls) {
        if (
          (el as HTMLElement).innerText?.trim() === "14" &&
          (el as HTMLElement).closest('[class*="cursor"]')
        ) {
    ...
    await expect(page.locator('button:has-text("Dia")').first()).toHaveClass(
      /bg-teal-600/,
    );
  });
});
...
test.describe("Calendar - Click to Create", () => {
  ...
  test("clicking empty slot should open create dialog", async ({ page }) => {
    ...
    await expect(
      page.getByRole("heading", { name: "Novo Agendamento", exact: true }),
    ).toBeVisible();
  });
  ...
  test("dialog should have all required form fields", async ({ page }) => {
    ...
    await expect(page.locator('[role="dialog"]')).toBeVisible();
    await expect(
      page.locator('[role="dialog"] input[id="patient"]'),
    ).toBeVisible();
    await expect(
    ...
    await expect(
      page.locator('[role="dialog"] input[id="notes"]'),
    ).toBeVisible();
    await expect(
      page.locator('[role="dialog"] button:has-text("Cancelar")'),
    ).toBeVisible();
    await expect(
      page.locator('[role="dialog"] button:has-text("Agendar")'),
    ).toBeVisible();
  });

  test("submit button should be disabled without patient name", async ({
    page,
  }) => {
    await clickSlot(page, 12, 0);
    await expect(page.locator('[role="dialog"]')).toBeVisible();
    ...
    const submitBtn = page.locator(
      '[role="dialog"] button:has-text("Agendar")',
    );
    await expect(submitBtn).toBeDisabled();
  });

  test("submit button should enable after filling patient name", async ({
    page,
  }) => {
    await clickSlot(page, 12, 0);
    await expect(page.locator('[role="dialog"]')).toBeVisible();
    ...
    await page
      .locator('[role="dialog"] input[id="patient"]')
      .fill("Paciente Teste E2E");
...
test.describe("Calendar - Event Interactions", () => {
  ...
  test("events in week view should be distributed across days", async ({
    page,
  }) => {
    await switchView(page, "Semana");
  ...
  test("professionals view should group events by dentist", async ({
    page,
  }) => {
    await switchView(page, "Profissionais");
...
test.describe("Calendar - CSS Grid Architecture", () => {
  ...
  test("time grid should have CSS variables for positioning", async ({
    page,
  }) => {
    ...
    const gridVars = await page.evaluate(() => {
      const gridArea = document.querySelector(
        '[style*="--hour-size"]',
      ) as HTMLElement;
  ...
  test("empty slots should be clickable", async ({ page }) => {
    const slot = page
      .locator("div.cursor-pointer[data-hour][data-minute]")
      .first();
    await expect(slot).toBeVisible();
...
test.describe("Calendar - Dark Mode", () => {
  ...
  test("calendar should render in dark mode without errors", async ({
    page,
  }) => {
...
test.describe("Calendar - Responsive", () => {
  ...
  test("view buttons should remain accessible on small screens", async ({
    page,
  }) => {
    await page.setViewportSize({ width: 640, height: 480 });
    await page.waitForTimeout(300);
    const viewButtons = page.locator(
      'button:has-text("Dia"), button:has-text("Semana")',
    );
...
test.describe("Calendar - List Toggle", () => {
  ...
  test("calendar should be default view", async ({ page }) => {
    ...
    const isActive = await calendarBtn.evaluate((el) =>
      el.classList.contains("bg-teal-100"),
    );
    expect(isActive).toBeTruthy();
  });
  ...
  test('clicking "Lista" should switch to list view', async ({ page }) => {
    ...
    const isActive = await listBtn.evaluate((el) =>
      el.classList.contains("bg-teal-600"),
    );
    expect(isActive).toBeTruthy();
  });
...
test.describe("Calendar - Data Loading", () => {
  ...
  test("should fetch appointments with correct date range for week view", async ({
    page,
  }) => {
    let capturedUrl = "";
  ...
  test("should fetch appointments for single day in day view", async ({
    page,
  }) => {
