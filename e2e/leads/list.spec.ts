...
t.describe('Leads Page', () => {
  ...
  t('renders Novo Lead button', async ({ page }) => { await expect(page.getByRole('button', { name: 'Novo Lead', exact: true })).toBeVisible({ timeout: 15000 }) })
  t('renders filters', async ({ page }) => { await expect(page.getByText('Status', { exact: true })).toBeVisible() })
})
