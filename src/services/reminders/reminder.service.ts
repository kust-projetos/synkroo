...
export async function processReminders(hoursBefore: number): Promise<{
	...
	for (const appointment of appointments) {
		...
		if (hoursBefore === 24) {
			...
			const description =
				...
				(appointment.dentistName
					? `👨‍⚕️ *Profissional:* Dr(a). ${appointment.dentistName}\n`
					: "") +
				(appointment.procedureName
					? `🦷 *Procedimento:* ${appointment.procedureName}\n`
					: "") +
		...
		if (result.success) {
			sent++;
			dbLogger.info(`Reminder sent to ${appointment.patientName}`, {
				phone: appointment.patientPhone,
			});
		} else {
			failed++;
			dbLogger.error(
				`Failed to send reminder to ${appointment.patientName}`,
				null,
				{ error: result.error },
			);
		}
...
export async function processAllReminders(): Promise<void> {
	...
	for (const config of DEFAULT_REMINDER_CONFIGS) {
		...
		dbLogger.info(`${config.hoursBefore}h reminders complete`, {
			sent: result.sent,
