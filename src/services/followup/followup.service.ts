...
export async function getAppointmentsNeedingFollowUp(
	...
	const { data: sentFollowUps } = await supabase
		...
		.in(
			"appointment_id",
			appointments?.map((a: FollowupAppointment) => a.id) || [],
		)
		.eq("feedback_type", "post_consultation");

	const alreadySent = new Set(
		sentFollowUps?.map((f: FeedbackRow) => f.appointment_id) || [],
	);
...
export async function processPostConsultationFollowUps(): Promise<{
	...
	for (const appointment of appointments) {
		...
		if (!config) {
			dbLogger.debug("No follow-up config found", {
		...
		if (result.success) {
			...
			failed++;
			console.error(
				`✗ Failed to send follow-up to ${appointment.patientName}: ${result.error}`,
			);
		}
	}
...
export async function getPatientsNeedingReturnReminder(
	...
	const targetDate = new Date(
		now.getTime() - monthsSinceLastVisit * 30 * 24 * 60 * 60 * 1000,
	);
    ...
    `)
		...
		.gte(
			"last_visit",
			new Date(targetDate.getTime() - 7 * 24 * 60 * 60 * 1000).toISOString(),
		)
	...
	for (const months of timeframes) {
		...
		for (const patient of patients) {
			...
			if (result.success) {
				totalSent++;
				dbLogger.info(
					`Return reminder sent to ${patient.name} (${months} months)`,
				);
			} else {
				totalFailed++;
				dbLogger.error(`Failed to send reminder to ${patient.name}`, null, {
					error: result.error,
				});
			}
		}
...
export async function processAllFollowUps(): Promise<void> {
	...
	dbLogger.info("Post-consultation complete", {
		sent: postResult.sent,
		failed: postResult.failed,
	});

	// Return reminders
	const reminderResult = await processReturnReminders();
	dbLogger.info("Return reminders complete", {
