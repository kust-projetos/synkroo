...
export async function deletePipelineStage(stageId: string): Promise<void> {
  ...
  if (count && count > 0) {
    ...
    if (!defaultStageId) {
      throw new Error('Cannot delete stage with leads and no default stage configured')
    }
...
export async function reorderPipelineStages(
  ...
  if (invalidIds.length > 0) {
    throw new Error(`Stage IDs do not belong to clinic: ${invalidIds.join(', ')}`)
  }
...
export async function seedDefaultPipelineStages(clinicId: string): Promise<void> {
  ...
  const defaults = [
    { name: 'Novo', color: '#3B82F6', sort_order: 0, is_system: false, system_key: null, is_default: true },
    { name: 'Contatado', color: '#8B5CF6', sort_order: 1, is_system: false, system_key: null, is_default: false },
    { name: 'Qualificado', color: '#10B981', sort_order: 2, is_system: false, system_key: null, is_default: false },
    { name: 'Proposta', color: '#F59E0B', sort_order: 3, is_system: false, system_key: null, is_default: false },
    { name: 'Negociação', color: '#EF4444', sort_order: 4, is_system: false, system_key: null, is_default: false },
    { name: 'Convertido', color: '#22C55E', sort_order: 5, is_system: true, system_key: 'converted', is_default: false },
    { name: 'Perdido', color: '#6B7280', sort_order: 6, is_system: true, system_key: 'lost', is_default: false },
  ]
