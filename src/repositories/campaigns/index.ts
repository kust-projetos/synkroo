...
export async function findCampaignsByClinic(
  clinicId: string,
): Promise<CampaignRow[]> {
  const db = getDb();
  const rows = await db
...
export async function findCampaignRecipients(
  campaignId: string,
...
export async function findPendingRecipients(
  campaignId: string,
): Promise<CampaignRecipientRow[]> {
  const db = getDb();
  const rows = await db
...
export async function findCampaignDispatchState(
  id: string,
...
export async function countCampaignRecipients(
  campaignId: string,
): Promise<number> {
  const db = getDb();
  const [row] = await db
...
export async function updateCampaignCounts(id: string): Promise<void> {
  ...
  const [sentCountRow] = await db
    ...
    .from(campaignRecipients)
    .where(
      and(eq(campaignRecipients.campaignId, id), sql`sent_at IS NOT NULL`),
    );
  ...
  const [responseCountRow] = await db
    ...
    .from(campaignRecipients)
    .where(
      and(eq(campaignRecipients.campaignId, id), sql`responded_at IS NOT NULL`),
    );
  ...
  const [conversionCountRow] = await db
    ...
    .from(campaignRecipients)
    .where(
      and(eq(campaignRecipients.campaignId, id), sql`converted_at IS NOT NULL`),
    );
...
export async function markRecipientResponded(
  id: string,
  content: string,
): Promise<CampaignRecipientRow | null> {
  return updateRecipientStatus(id, {
...
export async function markRecipientConverted(
  id: string,
...
export async function markRecipientError(
  id: string,
  errorMessage: string,
): Promise<CampaignRecipientRow | null> {
