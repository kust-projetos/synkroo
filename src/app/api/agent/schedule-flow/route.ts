...
import { findPatientById } from "@/repositories/patients";
...
export async function POST(request: NextRequest) {
	try {
		...
		const { message, patient_id, conversation_history = [] } = body;
...
async function handleSchedulingIntent(
	...
	if (!hasDate) {
		return NextResponse.json({
			action: "ask_date",
			message:
				'Claro! Vou te ajudar a agendar. Para qual data você gostaria? Pode me dizer algo como "amanhã", "quinta-feira" ou "dia 15".',
			intent: "agendamento",
			confidence: 0.9,
			entities,
	...
	if (!hasTime) {
		...
		const availability = await checkAvailabilityDirect(clinicId, date);
		...
		const morningSlots = availableSlots.filter(
			(s: { time: string }) => parseInt(s.time.split(":")[0]) < 12,
		);
		...
		if (morningSlots.length > 0) {
			slotsMessage += `☀️ **Manhã:** ${morningSlots
				.slice(0, 4)
				.map((s: { time: string }) => s.time)
				.join(", ")}\n`;
		}
		if (afternoonSlots.length > 0) {
			slotsMessage += `🌙 **Tarde:** ${afternoonSlots
				.slice(0, 4)
				.map((s: { time: string }) => s.time)
				.join(", ")}\n`;
		}
	...
	const slot = availability.slots.find(
		(s: { time: string; available: boolean }) => s.time === time && s.available,
	);
	...
	if (entities.procedimento) {
		const proceduresList = await findProceduresByClinic(clinicId, {
			activeOnly: true,
		});
		const procedure = (proceduresList as any[]).find((p) =>
			p.name.toLowerCase().includes(entities.procedimento!.toLowerCase()),
		);
	...
	const apptWithJoins = await findByIdWithJoins(created.id, clinicId);
	const appointment = apptWithJoins
		? {
				id: apptWithJoins.id,
				scheduled_at: apptWithJoins.scheduledAt.toISOString(),
				patients: apptWithJoins.patient
					? { name: apptWithJoins.patient.name }
					: null,
				procedures: apptWithJoins.procedure
					? { name: apptWithJoins.procedure.name }
					: null,
			}
		: null;
...
async function handleConfirmationIntent(
	...
	const appointment = await findUpcomingByPatient({
		clinicId,
		patientId,
	});
	...
	if (!appointment) {
		return NextResponse.json({
			...
			message:
				"Não encontrei nenhum agendamento pendente para confirmar. Quer agendar uma consulta?",
			intent: "confirmacao",
			entities,
		});
	}
...
async function handleEmergencyIntent(
	...
	return NextResponse.json({
		action: "escalate",
		message:
			"Entendi que você está passando por uma emergência. 😔 Vou transferir você imediatamente para um atendente que pode ajudar melhor. Por favor, aguarde um momento.",
		intent: "emergencia",
		confidence: 0.95,
		entities,
...
async function handleQuestionIntent(
	...
	// Get clinic procedures for context using Drizzle
	...
	const response = await llm.generateResponse(message, {
		...
		clinicInfo: {
			name: "Nossa clínica",
			procedures: procedureRows.map((p) => p.name),
		},
	});
