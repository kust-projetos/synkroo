/**
 * POST /api/cron/financeiro-collections — auth timing-safe antes de rate
 * limit + 429 canônico (padrão cleanup.ts).
 */
...
jest.mock('@/modules/financeiro', () => ({
  listOverdueCharges: jest.fn(),
  enrichOverdueCharges: jest.fn(),
  getCollectionStage: jest.fn(),
...
function makeRequest(auth?: string): Request {
  ...
  return new Request('http://localhost/api/cron/financeiro-collections', { method: 'POST', headers }) as any;
}
...
describe('POST /api/cron/financeiro-collections — auth gate', () => {
...
describe('POST /api/cron/financeiro-collections — rate limit', () => {
