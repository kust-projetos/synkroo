...
function makeGetRequest(): Request {
  return new Request('http://localhost/api/patients', { method: 'GET' }) as any;
}

function makePostRequest(): Request {
  return new Request('http://localhost/api/patients', { method: 'POST' }) as any;
}
...
describe('GET /api/patients — 429 rate limit bloqueado (D2)', () => {
...
describe('POST /api/patients — 429 rate limit bloqueado (D2)', () => {
  it('status 429 + envelope exato sem retryAfter no body + header Retry-After', async () => {
    mockRateLimit.allowed = false;
    mockRateLimit.remaining = 0;
    mockRateLimit.retryAfter = 30;
