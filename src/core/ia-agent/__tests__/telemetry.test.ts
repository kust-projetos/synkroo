...
describe('B2 — erro do orchestrator loga com correlationId e código estruturado', () => {
  ...
  it('execute_action negado no confirm loga código e mantém reply amigável', async () => {
    ...
    const r = await runTurn(
      ...
      {
        ...base,
        // Integração B1×B2: o confirm exige binding de principal (B1) — a
        // fixture carrega principalId nos dois lados para passar na reserva.
        pendingAction: { alias: 'a', args: {}, token, principalId: 'user-9' },
        principalId: 'user-9',
        confirmedToken: token,
      },
    );
    ...
    expect(r.errorCode).toBe('forbidden');
    // Integração: falha pós-reserva NÃO re-arma (clear condicional — B1).
    expect(r.pendingActionWrite).toBe('clear');
    const ev = events.find((e) => e.operation === 'execute_action');
    expect(ev).toMatchObject({
      correlationId: 'corr-123',
      status: 'error',
