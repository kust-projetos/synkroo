...
// ─── CRM module registry guard (Task 8) ───────────────────────────────────────

it('registers all CRM actions in the global registry', async () => {
  await bootstrapActions();
  const names = getActions().map((a) => a.name);
  expect(names).toContain('crm.listarContatos');
  expect(names).toContain('crm.obterContato');
  expect(names).toContain('crm.listarTimelineContato');
  expect(names).toContain('crm.listarNotasContato');
  expect(names).toContain('crm.adicionarNotaContato');
  expect(names).toContain('crm.atualizarTagsContato');
  expect(names).toContain('crm.listarSugestoesDuplicidade');
  expect(names).toContain('crm.obterSugestaoDuplicidade');
  expect(names).toContain('crm.aprovarSugestaoDuplicidade');
  expect(names).toContain('crm.dispensarSugestaoDuplicidade');
...
// ─── Financeiro module registry guard (Task 8) ────────────────────────────────

it('registers all financeiro actions in the global registry', async () => {
  await bootstrapActions();
  const names = getActions().map((a) => a.name);
  expect(names).toContain('financeiro.listarOrcamentos');
  expect(names).toContain('financeiro.obterOrcamento');
  expect(names).toContain('financeiro.criarOrcamento');
  expect(names).toContain('financeiro.listarParcelas');
  expect(names).toContain('financeiro.listarPagamentos');
  expect(names).toContain('financeiro.gerarCobranca');
  expect(names).toContain('financeiro.obterCobranca');
  expect(names).toContain('financeiro.cancelarCobranca');
  expect(names).toContain('financeiro.enviarLembreteCobranca');
  expect(names).toContain('financeiro.listarGateways');
});
