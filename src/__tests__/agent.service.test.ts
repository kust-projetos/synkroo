...
const {
	...
	createMessage,
	updateConversation,
} = require("../repositories/conversations");
...
function mockConversation() {
	...
	(findMessagesByConversation as jest.Mock).mockResolvedValue([
		{
			id: "msg-1",
		...
		},
		{
			...
			deliveredAt: null,
			readAt: null,
			createdAt: new Date("2026-03-27T10:00:05Z"),
		},
	]);
	(updateConversation as jest.Mock).mockResolvedValue(undefined);
}
...
describe("AgentService", () => {
	...
	describe("processMessage", () => {
		it("should process message and return response", async () => {
			...
			const result = await service.processMessage(
				"conv-123",
				"Quero marcar para quinta às 14h",
			);
	...
	describe("storeMessage", () => {
		...
		it("should handle database error when storing message", async () => {
			...
			await expect(Promise.reject(new Error("Database error"))).rejects.toThrow(
				"Database error",
			);
		});
	});
