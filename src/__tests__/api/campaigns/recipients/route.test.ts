...
describe('campaigns/[id]/recipients', () => {
  ...
  it('returns 403 for insufficient permission', async () => { mockValidateApiAuth.mockResolvedValue({ success: false, error: { message: 'Insufficient permissions', status: 403 } }); const r = await POST(new NextRequest('http://localhost', { method: 'POST', body: JSON.stringify({}) }), { params: Promise.resolve({ id: 'c1' }) }); expect(r.status).toBe(403) })
