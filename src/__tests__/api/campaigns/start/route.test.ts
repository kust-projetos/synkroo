...
const mockHasRequiredRole = jest.fn().mockReturnValue(true)
...
describe('campaigns/[id]/start', () => {
  ...
  it('returns 403 for insufficient role', async () => { authOk(); mockHasRequiredRole.mockReturnValue(false); const r = await POST(new NextRequest('http://localhost', { method: 'POST' }), { params: Promise.resolve({ id: 'c1' }) }); expect(r.status).toBe(403) })
  ...
  it('starts campaign successfully', async () => { authOk(); findCampaignById.mockResolvedValue({ clinicId: 'c1', status: 'draft' }); startCampaign.mockResolvedValue({ success: true }); const r = await POST(new NextRequest('http://localhost', { method: 'POST' }), { params: Promise.resolve({ id: 'c1' }) }); const b = await r.json(); expect(b.success).toBe(true) })
})
