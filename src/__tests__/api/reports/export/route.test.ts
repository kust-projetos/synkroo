...
describe('GET /api/reports/export',()=>{
  ...
  it('returns 400 for invalid type',async()=>{auth();const r=await GET(new Request('http://x?type=invalid')as any);const b=await r.json();expect(r.status).toBe(400);expect(b.error).toContain('Invalid')})
