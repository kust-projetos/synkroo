...
- **CI Production-mode E2E (atualizado 2026-09-25):** 3 causas-raiz encerradas em sequência (commits `cec81ba7`, `e6b2247e`, `8b2ee28c`): (1) seed E2E falhava com FK `role_permissions.permission_key → permissions.key` — `/api/seed` inseria grants sem popular o catálogo; fix: upsert idempotente de `comercialAccessPermissions` antes dos grants, com testes stateful (FK simulada + gate de ordem); (2) agregação `todayAppointments` do dashboard usava status `'pending'` inexistente no enum `appointment_status` (silenciado por catch que retornava zeros); fix: `= 'scheduled'`; (3) `/api/seed` rebindava o demo-admin de `Owner` (bypass total em `resolveAccess`) para `Administrador` com só 6 permissões comerciais — era no-op até 13/09 e virou bug ativo; fix: rebind removido, usuário preserva Owner. Suíte: 218/233 passando (era vermelha desde 2026-09-14 sem passar do seed). **Residuais (15 falhas, áreas independentes, invisíveis enquanto o seed quebrava):** patient detail (5 — aba "Informações" não renderiza), campaign list (2), CRM notes/lgpd-consent (2), accessibility-axe /login + /signup (2), logout (1), +3 a classificar. Follow-up: investigar cada área isoladamente; nenhuma bloqueia deploy (produção staging/prod com smoke verde). Gitleaks destravado 2026-09-25 (binário OSS 8.24.0 pinned, checksum-verified; cópia local em `%USERPROFILE%\bin` para o hook pre-commit; warning cosmético de BOM em `.gitleaksignore`).
