...
- **Contrato de API (D2 — fechado 2026-09-14):** envelope canônico completo (`src/services/api-handlers/**` + waitlist/appointments/patients/cron/knowledge); helper `apiRateLimited` em `src/lib/api/response.ts` (429 com `Retry-After` só em header); exceções deliberadas e remoção do shim `is_active` no `ADR-BASE-10`. Residuais menores: `instagram/webhook` e `messages/inbound` respondem 429 `{ error }` plano (decidir shape no ADR — restrição de retry do provedor); `widget/{session,messages}` emitem 429 em envelope sem header `Retry-After`; `cron/cleanup`, `crm/stats` e `auth/change-password` usam blocos manuais equivalentes ao helper; `handleApiError`/`RateLimitError` (`src/lib/errors.ts`) emitem shape legado sem header — sem uso em produção para 429 (candidatos a deprecação); união discriminada no retorno de `checkRateLimit` eliminaria o `?? 0` nos call sites.
...
- **Frontend (G1 residual):** predicates amplos de mutation/invalidação no frontend (over-invalidation — seguro, mas custoso); refinar para invalidação por clínica/recurso — desbloqueado com o fechamento do D2 (2026-09-14).
